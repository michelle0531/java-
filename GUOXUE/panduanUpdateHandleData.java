...
import java.awt.*;
import java.sql.*;
public class panduanUpdateHandleData implements ActionListener { 
   JTextField inputID;
   ...
   private void doGengxin(){
       ...
       try { String uri="jdbc:derby:GUOXUE;create=true"; 
             ...
             String SQL=
  "update panduanT set  number=?,content=?,pic=?,T=?,F=?,answer=?where number ='"+num+"'";
       ...
       if(m!=0){
        ...
        query.setSQL("SELECT * FROM panduanT where number='"+num+"'");
        query.startQuery();
